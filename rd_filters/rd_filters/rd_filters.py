...
def main():
    ...
    if cmd_input.get("template"):
        ...
        if input_file_name[-4:] == '.csv': 
            dfin = pd.read_csv(input_file_name)
            input_data = [(r.canonical_smiles,r.molecule_chembl_id) for i,r in dfin.iterrows()] 
        else: 
        ...
        if input_file_name[-4:] == '.csv': 
           merged_inner = pd.merge(left=dfin,right=df, left_on='canonical_smiles', right_on='SMILES')
           df = merged_inner
        df_ok = df[
            (df.FILTER == "OK") &
        ...
        output_smiles_file = prefix_name + "-clean.csv"
        output_csv_file = prefix_name + "-all.csv"
        df_ok.to_csv(f"{output_smiles_file}", sep=",", index=False, header=True)
        print(f"Wrote SMILES for molecules passing filters to {output_smiles_file}", file=sys.stderr)
        df[["SMILES", "NAME", "FILTER"]].to_csv(f"{output_csv_file}")
