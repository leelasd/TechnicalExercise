...
mols = []  
for glist in glob.glob('docked*.sdf'): 
    mols = mols+ list(oddt.toolkit.readfile('sdf', glist))
...
model_com = AgglomerativeClustering(affinity='precomputed', n_clusters=18, linkage='complete').fit(dists)
model_avg = AgglomerativeClustering(affinity='precomputed', n_clusters=18, linkage='average').fit(dists)
